...
let playerHealthPotions: number = 3;

const enemyStats: { [key: string]: { health: number; attackDamage: number } } = {
...
async function handleFight(enemy: string) {
...
async function selectEnemy() {
  const { enemy } = await inquirer.prompt({
    name: 'enemy',
    type: 'list',
    message: 'Choose your enemy to fight or run away:',
    choices: ['skeleton', 'zombie', 'warrior', 'assassin', 'Run Away'],
  });
...
async function main() {
  ...
  while (true) {
    ...
    if (enemy === 'Run Away') {
      console.log(chalk.bgRed.white.bold(`${playerName} ran away safely!`));
    } else {
